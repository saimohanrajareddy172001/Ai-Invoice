...
function categorizeItem(itemName) {
    ...
    // Supplies first — before Produce so "PINESOL LEMON" doesn't match Produce
    if (/\b(CONT\b|FOIL|BAG\b|GLOVE|TOWEL|BROOM|BLEACH|PINESOL|PINE.?SOL|CUP\b|LID\b|LINER|FILM\b|CHARMIN|GRILL BRICK|STRAW|WRAP|NAPKIN|SOAP|SANITIZER|SPONGE)\b/.test(n)) return 'Supplies';
    if (/^(PROD|PD)\b|\b(BROCCOLI|CAULIFLOWER|CILANTRO|CUCUMBER|CELERY|ZUCCHINI|TOMATILLO|MANGO|LEMON|LIME|BERRY|BERRIES|SPINACH|GINGER|MUSHROOM|TOMATO|CARROT|ONION|GARLIC|PEPPER|LETTUCE|ARUGULA|KALE|CABBAGE|RADISH|BEET|TURNIP|SQUASH|AVOCADO|EGGPLANT)\b/.test(n)) return 'Produce';
    ...
    if (/\b(OIL|FLOUR|SUGAR|STARCH|RICE|SALT|BASMATI|BEAN|PUREE|SAUCE|BATTER|KETCHUP|BAKING|PESTO|SRIRACHA|TERIYAKI|OLIVE|SPICE|TRUFFLE|GARBANZO|CORN|EVAP|VINEGAR|MAYO|MUSTARD|SOY|SEASONING|BREAD|BRD|PASTA|NOODLE|LENTIL|CHICKPEA)\b/.test(n)) return 'Dry Goods';
